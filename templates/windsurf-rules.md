# Inquisitor 自动审查规则 - Windsurf

本规则指导 Windsurf AI (Cascade) 在每次编码完成后自动调用 Inquisitor 进行严格的代码审查。
...
1. **After code changes**：当你完成了对代码文件的修改后，立即使用 MCP 工具 `review_diff` 审查本次变更的 git diff。
2. **审查新文件**：如果新增了文件且尚无 git diff，使用 `review_files` 工具，传入新增文件的路径列表。
3. **处理审查结果**：
...
在 Windsurf 的 MCP 配置中添加 Inquisitor：
